...
import { useGetClient, useGetCases } from "@workspace/api-client-react";
...
export default function ClientDetail() {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge variant="outline" className={`${s.color} font-medium`}>
        {s.label}
      </Badge>
    );
  };
  ...
  return (
    <div className="space-y-6">
      <Link
        href="/admin/clients"
        className="inline-flex items-center text-sm font-medium text-zinc-400 hover:text-amber-500 transition-colors"
      >
                    ...
                    <div className="block p-4 rounded-lg bg-zinc-900/30 border border-zinc-800 hover:border-amber-500/30 transition-colors group">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                          ...
                          <h4 className="font-medium text-zinc-200 group-hover:text-amber-500 transition-colors">
                            {c.title}
                          </h4>
